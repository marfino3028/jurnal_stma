...
<head>
    ...
    <title>Pemberitahuan perubahan status katalog Anda</title>
    <style type="text/css">
                ...
                <tbody><tr>
                    <td>
                        <table border="0" align="center" width="460" cellpadding="0" cellspacing="0" class="container590 bodybg_color">
                            ...
                            <tr>
                                <td>
                                    <table border="0" align="" cellpadding="0" cellspacing="0" class="container580">
                                        <tbody><tr>
                                            <td align="" style="color: #717f8c; font-size: 14px; font-family: 'Open Sans', sans-serif; mso-line-height-rule: exactly; line-height: 24px;" class="resize-text text_color">
                                                <div class="editable_text" style="line-height: 24px">
														<span class="text_container">
														<!-- ======= section text ======= -->
                                                        <p>Terdapat update status dari katalog Anda yang diunggah di {{ config('app.name') }} dengan detail sebagai berikut </p>
                                                        <table class="table" width="100%">
                                                            @foreach ($catalog->catalog_metadata_value as $item)
                                                            <tr>
                                                                <th style="text-align: left" class="col-md-2">{{$item->metadata->value}}</th>
                                                                <td class="col-md-4">{{$item->value}}</td>
                                                            </tr>    
                                                            @endforeach
                                                        </table>
                                                        <p>Silakan review untuk melihat status unggahan tesebut pada link <a href="{{route('catalogs.show',$catalog->id)}}">{{route('catalogs.show',$catalog->id)}}</a> </p>
                                                        </span>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <tbody><tr>
                                <td align="center" style="color: #ffffff; font-size: 14px; font-family: 'Open Sans', sans-serif; line-height: 22px;">
                                    <div class="editable_text" style=" line-height: 22px;">
			        						<span class="text_container">
			            					<a href="{{route('catalogs.show',$catalog->id)}}" style="color: #ffffff; text-decoration: none;" class="text2_color">{{route('catalogs.show',$catalog->id)}}</a>
			        						</span>
                                    </div>
                                </td>
                            </tr>
